...
x0_mean = np.concatenate((x0_pos, x0_vel, x0_musclestate, x0_target))
...
# 2. Solve problem
print(f"Compute LQG trajectory for N={N}, dt={dt}, x0_mean={x0_mean}, Sigma0={Sigma0}, and T={x0_target}.\n"
...
plt.figure(0)
plt.title(f"LQG Position Time Series")
plt.xlabel("Time (s)")
...
plt.figure(1)
plt.title(f"LQG Velocity Time Series")
plt.xlabel("Time (s)")
...
plt.figure(2)
plt.title(f"LQG Control Time Series")
plt.xlabel("Time (s)")
plt.ylabel("Control")
